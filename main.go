...
func Probe(registry *prometheus.Registry, client *client.DevtoClient) (success bool) {
	...
	for _, a := range articles {
		...
		totalViewsGauge.WithLabelValues(id).Set(float64(a.PageViewsCount))
		totalReactionGauge.WithLabelValues(id).Set(float64(a.PublicReactionsCount))
		totalCommentsGauge.WithLabelValues(id).Set(float64(a.CommentsCount))
	...
	totalArticlesGauge.Set(float64(totalPublishedArticles))
